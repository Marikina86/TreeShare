    ...
    <div className="mb-6 p-5 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-2xl">
      ...
      {campaign.expiresAt && (
        <div className="flex items-center gap-4 text-xs text-emerald-600 dark:text-emerald-400">
          {campaign.durationDays && <span>{campaign.durationDays} {l.days}</span>}
          <span>{l.expiresOn}: {new Date(campaign.expiresAt).toLocaleDateString(lang === "it" ? "it-IT" : lang === "fr" ? "fr-FR" : lang === "pt" ? "pt-PT" : lang === "es" ? "es-ES" : lang === "ja" ? "ja-JP" : "en-US", { day: "2-digit", month: "short", year: "numeric" })}</span>
        </div>
      )}
    </div>
  );
